...
file: sertcp.c rev1 by Dan Lindamood
purpose: Capture serial text from serial port and also
forward to tcp port for printer 9100 telnet.

To compile, add this file along with 
rs232.c, rs232.h and socket.h 
together into a directory. 

Use command gcc sertcp.c rs232.c -o sertcp

run sertcp with 2 arguments (Printer IP and Close Socket timeout). 
example: ./sertcp 192.168.0.100 90
...
int main (int argc, char *argv[]) {
  ...
  int check_com = 0,
      ...
      n = 0,
      timeout = 0,
      cport_nr=23,        /* open rs232.c and from the list of serial ports, enter the location. Ex.23 = /dev/ttyAMA0 (COM1 on windows) */
